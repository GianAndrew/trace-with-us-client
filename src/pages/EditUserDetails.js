...
const EditUserDetails = () => {
	...
	const [zipCode, setZipCode] = useState(auth.zipCode);

	const handleUpdate = async () => {
		try {
			const { data } = await axiosApi.post('/visitors/update', {
				id: parseInt(id),
				name: name,
				email: email,
				age: parseInt(age),
			...
			localStorage.setItem('user', JSON.stringify({ ...auth, id: id, email: email, name: name, age: age, gender: gender, phone: phone, address: address, zipCode: zipCode }));

			navigate('/visitors/userdetails');
			return data;
		} catch (error) {
				...
				<div className='container mx-auto bg-white h-screen px-10 md:px-4 lg:px-10 xl:px-12 flex items-center justify-center'>
					<div className='h-full w-full flex items-stretch justify-start flex-col mt-4'>
						...
						<h1 className='text-4xl font-bold text-blue-500 my-5'>
							Trace With Us <span className='text-black'>Edit User Details Page</span>
						</h1>
